...
import AuctionsShop from '../AuctionsShop';

class AuctionShop extends React.PureComponent {
  componentDidMount() {
  ...
  render() {
    const { products, advancedFilters, filterAuctions } = this.props;
      ...
      <div className='shop'>
        <Row xs='12'>
          {/* <Col
            ...
            lg={{ size: 3, order: 1 }}
          >
            <ProductFilter filterProducts={filterAuctions} />
          </Col> */}
          <Col
            xs={{ size: 12, order: 2 }}
            sm={{ size: 12, order: 2 }}
          ...
          >
            <Row className='align-items-center mx-0 mb-4 mt-4 mt-lg-0 py-3 py-lg-0 bg-white shop-toolbar'>
              ...
              >
                <span>Showing: </span>
                {totalProducts > 0
                  ? `${left}-${right} Auction of ${count} Auctions`
                  : `${count} Auctions`}
              </Col>
              <Col
                xs={{ size: 12, order: 2 }}
              ...
              >
                <SelectOption
                  ...
                  handleSelectChange={(n, v) => {
                    filterAuctions('sorting', n.value);
                  }}
                />
              </Col>
            </Row>
            <Switch>
              <Route exact path='/auction' component={AuctionsShop} />
...
const mapStateToProps = state => {
  return {
    advancedFilters: state.auction.advancedFilters,
    products: state.auction.storeAuctions
  };
};

export default connect(mapStateToProps, actions)(AuctionShop);
