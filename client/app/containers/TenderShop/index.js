...
import TendersShop from '../TendersShop';

class TenderShop extends React.PureComponent {
  componentDidMount() {
  ...
  render() {
    const { products, advancedFilters, filterTenders } = this.props;
          ...
          >
            <Row className='align-items-center mx-0 mb-4 mt-4 mt-lg-0 py-3 py-lg-0 bg-white shop-toolbar'>
              ...
              >
                <span>Showing: </span>
                {totalProducts > 0
                  ? `${left}-${right} Tender of ${count} Tenders`
                  : `${count} Tenders`}
              </Col>
              <Col
                xs={{ size: 12, order: 2 }}
              ...
              >
                <SelectOption
                  ...
                  handleSelectChange={(n, v) => {
                    filterTenders('sorting', n.value);
                  }}
                />
              </Col>
            </Row>
            <Switch>
              <Route exact path='/tender' component={TendersShop} />
...
const mapStateToProps = state => {
  return {
    advancedFilters: state.tender.advancedFilters,
    products: state.tender.storeAuctions
  };
};

export default connect(mapStateToProps, actions)(TenderShop);
